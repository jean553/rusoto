...
impl DispatchSignedRequest for Client {
    fn dispatch(&self, request: &SignedRequest) -> Result<HttpResponse, HttpDispatchError> {
        ...
        let mut final_uri = match request.endpoint_url().is_empty() {
            true => format!("https://{}{}", request.hostname(), request.canonical_path()),
            false => format!("{}{}", request.endpoint_url(), request.canonical_path())
        };
